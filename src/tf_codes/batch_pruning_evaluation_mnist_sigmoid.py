...
rp.robust_pruning(mode='baseline', size=SAMPLES, benchmarking=benchmarking_mode, activation=activation, adversarial_epsilons=[0.01, 0.05, 0.1])

for index, item in enumerate(hyperparameters):
    rp.robust_pruning(mode='stochastic', size=SAMPLES, benchmarking=benchmarking_mode, activation=activation, alpha=item, adversarial_epsilons=[0.01, 0.05, 0.1])
