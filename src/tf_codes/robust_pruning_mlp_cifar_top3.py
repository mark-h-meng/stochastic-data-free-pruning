...
def load_cifar_image_labeling_dataset(normalize=True):
...
def robust_pruning(mode='entropy', size=100, alpha=0.5, activation='relu', benchmarking=0,
    ...
    if curr_mode == 'baseline':
        ...
        print('>' * 50)
        print(">> CASE: CIFAR-10 Top-K ("+activation+")")
        ...
        print(bcolors.OKCYAN)
        print('>' * 50)
        print(">> CASE: CIFAR-10 Tok-K ("+activation+")")
    ...
    original_model_path = 'tf_codes/models/cifar_10_topk_cnn'
    pruned_model_path = 'tf_codes/models/cifar_10_topk_cnn_pruned'
    ...
    # There are 50000 training images and 10000 test images.
    (train_images, train_labels), (test_images, test_labels) = load_cifar_image_labeling_dataset(normalize=True)
    print("Training dataset size: ", train_images.shape, train_labels.shape)


    optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
    
    training_from_data.train_cifar_9_layer_cnn((train_images, train_labels),
    ...
    loss, accuracy, top_k_accuracy = model.evaluate(test_images, test_labels, verbose=2)
    ...
    tape_filename = "tf_codes/logs/cifar10--top3-" + activation + "-" + timestamp + "-" + str(BATCH_SIZE_PER_EVALUATION)
    if BENCHMARKING_MODE:
