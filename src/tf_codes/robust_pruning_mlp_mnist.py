...
TARGET_ADV_EPSILONS = [0.01, 0.05, 0.1]
...
def main(args):
    ...
    if TRAIN_BIGGER_MODEL:
        original_model_path = 'models/mnist_mlp_5_layer'
        pruned_model_path = 'models/mnist_mlp_pruned_5_layer'
    else:
        original_model_path = 'models/mnist_mlp'
        pruned_model_path = 'models/mnist_mlp_pruned'
    ...
    ################################################################
    tape_filename = "logs/" + timestamp + "-" + str(BATCH_SIZE_PER_EVALUATION) + "-" + activation
    if BASELINE_MODE:
