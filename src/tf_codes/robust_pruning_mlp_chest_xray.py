...
def robust_pruning(mode='entropy', size=100, alpha=0.75, activation='relu', benchmarking=0,
    target_pruning_percentage=0.8, adversarial_epsilons= [0.5], batch_size_per_shot=4, pooling_multiplier=2):
    curr_mode = mode
    ...
    if curr_mode == 'baseline':
        ...
        print('>' * 50)
        print(">> CASE: CHEST X-RAY DETECTION")
    ...
    tape_filename = "tf_codes/logs/chest-" + timestamp + "-" + str(BATCH_SIZE_PER_EVALUATION)
    if BENCHMARKING_MODE:
